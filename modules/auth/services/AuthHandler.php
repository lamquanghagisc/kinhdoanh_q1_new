            ...
            if ($auth) {
                ...
                if ($email == null) {
                    ...
                    $user = new AuthUser([
                        'username' => $email,
                        'fullname' => $nickname,
                        'auth_role_id' => AuthUserService::$AUTH_ROLE['USER'],
                        'status' => AuthUserService::$AUTH_STATUS['ACTIVE'],
                        'delete' => AuthUserService::$AUTH_DELETE['ALIVE'],
                        'confirmed' => AuthUserService::$AUTH_CONFIRM['CONFIRMED'],
                        'type' => AuthUserService::$AUTH_TYPE['PUBLIC'],
                    ...
                    if ($user->save()) {
                        ...
                        Yii::$app->session->setFlash('error', Yii::t('app', 'Unable to save ' . $this->client->getTitle() . ' account: ' . json_encode($user->getErrors())));
                    }
                }
            }
        } else {
